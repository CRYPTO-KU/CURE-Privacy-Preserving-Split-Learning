...
X_train = X_train.reshape(X_train.shape[0], 1, 23)
y_train = np.eye(2)[y_train.astype(int)]

X_test = X_test.reshape(X_test.shape[0], 1, 23)
y_test = np.eye(2)[y_test.astype(int)]

random.seed(time.time())

parser = argparse.ArgumentParser()
parser.add_argument("--input", type=int, default=23, help="input controls the number of input nodes")
parser.add_argument("--hidden", type=str, default="128,32", help="output controls the number of hidden nodes (comma-separated)")
parser.add_argument("--output", type=int, default=2, help="output controls the number of output nodes")
